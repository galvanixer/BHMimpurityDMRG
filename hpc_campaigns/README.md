...
- If the effective `initial_state.impurity_distribution` is fixed to a deterministic mode (`centered_pileup`, `manual`, `centered_block`, `uniform_spread`, `two_cluster`, or `asymmetric_mixed`), `launch_campaign.jl` ignores `initial_state.seed` entirely:
